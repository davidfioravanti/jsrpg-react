...
// ==========================================================================
// ==================== ENCOUNTER CONFIG (SETTINGS/ETC) =====================
// ==========================================================================

const preDefTimeout = 1000;


// SOUND CONFIG (VIA OPTIONS SCREEN IN TRAVERSE.HTML)
...
localStorage.setItem("seenPumpking", true);
localStorage.setItem("currentEnemy", "PUMPK1N6");
localStorage.setItem("lastScreen", "pumpking.js");
...
$(pumpkingSound).animate({
    volume: musicVolume
}, 1000, function () {
});

pumpkingSound.loop = true;

pumpkingSound.onended = function () {
    combatSound.play();
}
...
setTimeout(() => {
    ...
    if (!turnNum === null || currentEnemyHp < 80) {
        $(".introText").addClass("container");
        $(".introText").html("<h1 class='text-center flash'>CONTINUE FIGHTING<br>"
            + currentEnemy + " LEVEL 5</h1>")
    }
...
function enemyAttack() {
    var enemyAttackResponses = [
        currentEnemy + " hypnotizes you into attacking yourself!",
        currentEnemy + " mesmerizes you with it's hypnotic gaze.",
    ]
    ...
    setTimeout(() => {
        $(".pumpkin1").removeClass("pumpkinAttack");
        $(".handLeft").removeClass("handLeftAttack");
        $(".handRight").removeClass("handRightAttack");
        $(".hypnotizeDiv").fadeOut();
        $("#actionsDiv").show();
...
function rollEnemyDamage() {
    var eMin = Math.ceil(4);
    var eMax = Math.floor(11);
